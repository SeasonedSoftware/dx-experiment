...
const markStoryReady = stories.markStoryReady

describe('markStoryReady', () => {
  ...
  it('throws an error if a story is not found', async () => {
    ...
    expect.assertions(1)
    try {
      await markStoryReady.run(params)
    } catch (e) {
      expect(e.message).toMatch('story_ready_story_id_fkey')
  ...
  it('approves scenario if a valid id is given', async () => {
    ...
    await markStoryReady.run(params)
    expect(
      await getPrisma().storyReady.count({
